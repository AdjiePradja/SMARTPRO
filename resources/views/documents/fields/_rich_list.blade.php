...
<div class="mb-4" x-data="richList(@js($items), '{{ $prefix }}')">
    ...
    <template x-for="(item, i) in items" :key="i">
        <div class="input-group mb-2">
            <span class="input-group-text font-monospace" x-text="label(i)" x-show="'{{ $prefix }}'"></span>
            <input type="text" class="form-control" name="sections[{{ $key }}][]" x-model="items[i]"
                   @if($suggestions) list="dl-{{ $key }}" @endif placeholder="{{ $suggestions ? 'Masukkan referensi...' : 'Masukkan poin...' }}">
            <button type="button" class="btn btn-outline-danger" @click="remove(i)" title="Hapus"><i class="bi bi-x-lg"></i></button>
        </div>
    </template>
